...
fn main() {
    ...
    let output_size = if let Some(size) = env_args.get(2) {
        size.parse::<usize>().unwrap_or(30)
    } else {
        30
    };

    let words = POEM.split_whitespace().collect::<Vec<_>>();
    ...
    let mut counter = 0;
    while !finished {
        if let Some(predicted) = chain.generate(&buffer) {
            print!(" {}", predicted);
            if predicted.ends_with('.') || predicted.ends_with('?') || predicted.ends_with('!') {
                print!("\n");
